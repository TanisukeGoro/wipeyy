  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(['bindVideoReferrer'], function(result) {
      ...
      chrome.tabs.query({}, function(tabs) {
        // 存在しないタブがあるか検索
        const nilTabIds = videoItems.map(tab => tab.tabId).filter(n => tabs.map(tab => tab.id).indexOf(n) === -1)
    ...
    <div>
      {items.map((item, index) => (
        <div key={index} className="d-flex">
          <div className="browser-view" style={{ backgroundColor: linkedTabId === item.tabId ? '#a6d06f' : '' }}>
